...
class AnuSpiderSpider(scrapy.Spider):
    ...
    def parse_course_detail(self, response):
        ...
        if any(keyword in course_name for keyword in exclude_keywords):
            return
        ...
        tuition_fee_raw = response.css(
            "#indicative-fees__international dl dd::text"
        ).get()
        tuition_fee = (
            tuition_fee_raw.replace("$", "").replace(",", "").replace(".00", "").strip()
        )

        duration_text = response.css(
            "li.degree-summary__requirements-length span.tooltip-area::text"
        ).get()
