...
class MurdochSpiderSpider(scrapy.Spider):
    ...
    async def parse(self, response):
        ...
        while True:
            try:
                next_page = await page.wait_for_selector(
                    "ul.pagination .page-item span.fal.fa-arrow-right",
                    state="visible",
                    timeout=1000,
                )
            except:
    ...
    async def parse_course_page(self, response):
        ...
        # 抓取學費
        tuition_fee_raw = response.xpath(
            '//div[@class="is-international"][2]/dd/text()'
        ).get()
        tuition_fee = (
            tuition_fee_raw.replace("$", "").replace(",", "")
            if tuition_fee_raw
            else None
        )
    ...
    def extract_course_url(self, response):
        ...
        for card in cards:
            ...
            if any(keyword in course_name for keyword in keywords) and all(
                keyword not in course_name for keyword in exclude_keywords
            ):
